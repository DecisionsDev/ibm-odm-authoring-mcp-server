...
Here is the list of the tools with their `operationId` and tag/category in ODM 9.5.0.1:

  | `operationId` | tool      | tag     |
  |---------------|-----------|---------|
  | registerWebhook | Register a webhook to notify other applications of events that are coming from Decision Center | ['Manage'] |
  | renameSnapshot | Rename a snapshot from a decision service | ['Manage'] |
  | addServer | Add a target server to use for deployments, simulations, and tests | ['Manage'] |
  ...
  | setPersistenceLocale | Set persistence locale | ['DBAdmin'] |
  | registerWebhook_1 | Update a webhook to notify other applications of events that are coming from Decision Center | ['Manage'] |
  | deleteWebhook | Unregister a webhook | ['Manage'] |
  | addUser | Add or update a user | ['Admin'] |
  | run | Run a test suite | ['Build'] |
  | server | Details of the server | ['Explore'] |
  | updateServer | Update a target server to use for deployments, simulations, and tests | ['Manage'] |
  | deleteServer | Remove a target server to use for deployments, simulations, and tests | ['Manage'] |
  | getUsersRolesRegistry | Retrieve the last configuration file that was uploaded | ['Admin'] |
  | setUsersRolesRegistry | Set the configuration for users, groups and roles | ['Admin'] |
  | ldapSync | Synchronize the repository with any associated LDAP server | ['Admin'] |
  ...
  | importTabPermissions | Import tab permissions | ['Admin'] |
  | importPermissions | Import permissions | ['Admin'] |
  | importCommandPermissions | Import command permissions | ['Admin'] |
  | addGroup | Add or update a group | ['Admin'] |
  ...
  | branchSecurity | Security configuration of a branch | ['Admin'] |
  | branchSecurity_1 | Enforce the security on a branch | ['Admin'] |
  ...
  | uploadMessagesFile | Persist localized messages file | ['DBAdmin'] |
  | uploadExtensionModelFiles | Persist model extension files .brmx and .brdx | ['DBAdmin'] |
  | executeSQLScript | Run SQL script | ['DBAdmin'] |
  | generate | Launch DC database diagnostics generation | ['DBAdmin'] |
  | generateExtensionModelScript | Generate an SQL script for model extensions | ['DBAdmin'] |
  | stopCleanup | Stop cleanup operation | ['DBAdmin'] |
  | launchCleanup | Launch cleanup of the repository | ['DBAdmin'] |
  | webhooks | Get a list of the webhooks that are bound to this instance of Decision Center | ['Manage'] |
  | users | List of the users that are defined in Decision Center | ['Admin'] |
  | eraseAllUsers | Remove all users | ['Admin'] |
  | user | Details of the user | ['Admin'] |
  | deleteUser | Remove a user | ['Admin'] |
  ...
  | metrics | Get repository metrics | ['Admin'] |
  ...
  | exportTabPermissions | Export the tab permissions to JSON | ['Admin'] |
  | exportPermissions | Export the permissions that are defined for a group to JSON | ['Admin'] |
  | effectivePermissions | Retrieve the effective permissions for one or more groups to JSON | ['Admin'] |
  | exportCommandPermissions | Export the command permissions to JSON | ['Admin'] |
  | groups | List of the groups that are defined in Decision Center | ['Admin'] |
  | eraseAllGroups | Remove all groups | ['Admin'] |
  | group | Details of the group | ['Admin'] |
  | deleteGroup | Remove a group | ['Admin'] |
  ...
  | deleteDecisionService | Delete a decision service | ['Admin'] |
  ...
  | branch_1 | Details of the branch from which the security configuration is inherited from | ['Admin'] |
  | branchGroups | Comma-separated list of the groups that are set on a branch | ['Admin'] |
  | about | Get system, product, and database information | ['About'] |
  | getExecutionStatus | Get the run status of the SQL script | ['DBAdmin'] |
  | getModelExtensionFiles | Retrieve model extension files as file archive | ['DBAdmin'] |
  | generateMigrationRole | Generate a migration role | ['DBAdmin'] |
  | generateMigrationScript | Generate migration script | ['DBAdmin'] |
  | results | Get DC database diagnostics results | ['DBAdmin'] |
  ...
  | discardBuildState | Discard all the built states of branches, releases, activities, and snapshots | ['Manage'] |
